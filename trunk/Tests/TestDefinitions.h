...
TEST(LetSingle) {
	...
	Term * z = Redex(MakeList(3, LazyFunction(LazyFunctionLet), a, b));
...
TEST(DefineVariable) {
	ContextBindings * contextBindings = AcquireContextBindings();
	Term * z = Redex(MakeList(3, LazyFunction(LazyFunctionDefine), SymbolFromConstantStr(STR("lalala")), Number(10)));
...
TEST(DefineVariableEvaluatesValue) {
	ContextBindings * contextBindings = AcquireContextBindings();
	Term * z = Redex(MakeList(3, LazyFunction(LazyFunctionDefine), SymbolFromConstantStr(STR("lalala")), Redex(MakeList(3, Function(OperatorPlus), Number(10), Number(20)))));
	AssertEq(Empty(), Eval(z, contextBindings));
...
TEST(DefineWithNumberAsName) {
	ContextBindings * contextBindings = AcquireContextBindings();
	Term * z = Redex(MakeList(3, LazyFunction(LazyFunctionDefine), ParseSingle("10"), ParseSingle("(+ p1 p2)")));
	AssertEq(InvalidArgumentType(), Eval(z, contextBindings));
}

TEST(DefineWithNumberAsFunctionName) {
	ContextBindings * contextBindings = AcquireContextBindings();
	Term * z = Redex(MakeList(3, LazyFunction(LazyFunctionDefine), ParseSingle("(10 p1 p2)"), ParseSingle("(+ p1 p2)")));
	AssertEq(InvalidArgumentType(), Eval(z, contextBindings));
}
